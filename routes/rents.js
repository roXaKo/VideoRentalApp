...
router.delete('/:customer/:movie', auth, async (req, res) => {
    ...
    const result = await Rents.lookup(req.params.customer, req.params.movie, { _id: 1 })
    if (!result) return res.status(404).send('rental not found')

    await Rents.findByIdAndDelete(result)
    await Movie.updateOne({ _id: req.params.movie }, { $inc: { numberInStock: 1 } })
...
router.put('/:customer/:movie', auth, async (req, res) => {
    ...
    let result = await Rents.lookup(req.params.customer, req.params.movie, { customer: 1, title: 1, dateReturned: 1 })
    if (!result) return res.status(404).send("rental not found")
    if (result.dateReturned) return res.status(400).send("rental allready returned")
