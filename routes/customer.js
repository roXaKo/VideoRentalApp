...
router.get('/', async (req, res) => {
    res.send(await Customers.find().select({ name: 1, _id: 1, phone: 1, isGold: 1 }).sort({ _id: 1 }))})

router.get('/:_id', async (req, res) => {
    
    res.send(await Customers.findOne({ _id: req.params._id }).select({ name: 1, _id: 1, phone: 1, isGold: 1 }))})
...
router.post('/new', auth, async (req, res) => {
    ...
    if (await Customers.findOne({ name: name })) return res.status(400).send('name already exists')
    if (await Customers.findOne({ phone: phone })) return res.status(400).send('phoneNr already exists')
...
router.delete('/:_id', auth, async (req, res) => {
    const result = await Customers.findOne({ _id: req.params._id }).select({ _id: 1 })
...
router.put('/:_id', auth, async (req, res) => {
    ...
    if (typeof (isGold) !== "boolean" && isGold !== null) return res.status(400).send('isGold needs to be true or false')
    if (await Customers.findOne({ name: name })) return res.status(400).send('name already exists')
    if (await Customers.findOne({ phone: phone })) return res.status(400).send('phoneNr already exists')

    let result = await Customers.findOne({ _id: req.params._id }).select({ name: 1, isGold: 1, phone: 1 })

    if (req.body.name) result.name = name
    if (req.body.isGold) result.isGold = isGold
    if (req.body.phone) result.phone = phone

    await Customers.findOneAndReplace({ _id: req.params._id }, { name: result.name, isGold: result.isGold, phone: result.phone })
    res.send(result)
})
